...
document.addEventListener("DOMContentLoaded", function () {
  var app = new vue_dist_vue_esm__WEBPACK_IMPORTED_MODULE_0__["default"]({
    ...
    methods: {
      ...
      prevForm: function prevForm() {
        if (this.formIndex > 0) {
          this.formIndex -= 1;
        } else {// Disabled button
        }
      },
      addExam: function addExam(e) {
        var regexp, time;
        time = new Date().getTime();
        regexp = new RegExp(e.target.dataset["id"], "g");
        $(this.$refs.exams).append(e.target.dataset["fields"].replace(regexp, time));
        return e.preventDefault();
      },
      addInterest: function addInterest(e) {
        var regexp, time;
        time = new Date().getTime();
        regexp = new RegExp(e.target.dataset["id"], "g");

        if (this.$refs.interests.childElementCount < 5) {
          $(this.$refs.interests).append(e.target.dataset["fields"].replace(regexp, time));
        } else {
...
/******/ });
//# sourceMappingURL=signup_form-deeb34564b6c2d543a2e.js.map